...
public interface TypeConverterManager {
	 ...
	 *             如果 sourceType 或者 targetType 或者 typeConverter 为 null。
	 */
	 ...
	 *            需要注册的类型转换器实例。
	 * @throws IllegalArgumentException
	 *             如果 typeConverter 为 null 或者提供的源类型或者转换到的目标类型为 null 。
	 */
	 ...
	 * 
	 * @param sourceType
	 *            需要解除注册的转换的源类型的 {@link Class}。
	 * @param targetType
	 *            需要解除注册的转换到的类型的 {@link Class}。
	 ...
	 *             如果没有找到可完成 source 到 targetType 的转换器。
