...
public class MultiMybatisAutoConfig {
    ...
    @Primary
	public SqlSessionFactory sqlSessionFactory(MybatisProperties mybatisProperties,@Qualifier("dataSource") DataSource dataSource) throws Exception {
	...
	private static MybatisAutoConfig createMybatisAutoConfig(){
        ConfigurableListableBeanFactory factory = ((AbstractApplicationContext) ApplicationContextHolder.get())
            .getBeanFactory();
        ...
        Interceptor[] interceptors = factory.getBeansOfType(Interceptor.class).values().toArray(new Interceptor[0]);
	...
	static class Initializer implements ImportBeanDefinitionRegistrar {
		...
		public void registerBeanDefinitions(AnnotationMetadata importingClassMetadata,
			...
			if (mybatisProperties.isSupportMultiDataSource()) {
				for (Map.Entry<String, MybatisProperties.Multi> entry : mybatisProperties.getMulti().entrySet()) {
					if (!entry.getValue().isPrimary()) {
						DruidDataSource druidDataSource = DruidProperties.buildFromEnv(entry.getValue().getDsPrefix());
						String dataSourceName = entry.getKey() + "DataSource";
						factory.registerSingleton(dataSourceName, druidDataSource);
						SqlSessionFactory sqlSessionFactory = createMybatisAutoConfig().sqlSessionFactory(druidDataSource,
							mybatisProperties);
                        ...
                        factory.registerSingleton(pagedJdbcTemplateName, new PagedJdbcTemplate(druidDataSource));
					}
				}
			}
		}
