...
public class ResourceServiceImpl extends EntityServiceImpl<Resource, ResourceDao> implements ResourceService {
	...
	public List<ResourceNode> getAuthorizedResourceNode(Long userId, Long roleId) {
		...
		if (roleId != null) {
			List<ResourceNode> roleResources = getEntityDao().getResourcesNodeByRole(Long.valueOf(roleId));
	...
	private ResourceNode convertResource(Resource r) {
		ResourceNode node = new ResourceNode(r.getId(), r.getParent() != null ? r.getParent().getId() : null,
				r.getName(), r.getType(), r.getShowMode(), r.getValue(), r.getIcon());
