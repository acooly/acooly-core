...
public class SimpleUploadServlet extends HttpServlet {
    ...
    private static final Logger logger = LoggerFactory.getLogger(SimpleUploadServlet.class);
    ...
    public void init(ServletConfig config) throws ServletException {
        ...
        if (StringUtils.isNotBlank(config.getInitParameter("allowedExtensions"))) {
        ...
        if (StringUtils.isNotBlank(config.getInitParameter("needRename"))) {
            needRename =
                    StringUtils.equalsIgnoreCase(config.getInitParameter("needRename"), "true")
                            ? true
                            : false;
        }
        logger.info(
                "Init parameters [root:{} ,accessRoot: {}, allowedExtensions:{}, uploadFieldName:{} ,needRename={}]",
                root,
                accessRoot,
                allowedExtensions,
                uploadFieldName,
    ...
    protected File doUpload(HttpServletRequest request, HttpServletResponse response)
            throws IOException {
        ...
        try {
            ...
            if (needRename) {
                fileName = getFileRename() + "." + getFileExtension(requestFileName);
            } else {
    ...
    protected void postUpload(
            HttpServletRequest request, HttpServletResponse response, File uoloadFile)
            throws IOException {
    }
    ...
    protected String getFileRename() {
        return Dates.format(Dates.DATETIME_NOT_SEPARATOR) + RandomStringUtils.randomNumeric(4);
    }
    ...
    protected String getAccessUrl(HttpServletRequest request, File uploadFile) {
        ...
        String relativePath = StringUtils.substringAfter(localFilePath, getStorageRoot(request));
