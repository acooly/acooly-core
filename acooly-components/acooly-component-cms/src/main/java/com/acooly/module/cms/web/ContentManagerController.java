...
public class ContentManagerController
	 ...
	 * 分页
	 */
	protected PageInfo<Content> doList(
            HttpServletRequest request, HttpServletResponse response, Model model)
		throws Exception {

		Map<String, Object> params = getSearchParams(request);
		return getEntityService().query(
			getPageInfo(request), params, getSortMap(request));
	}

	/**
	 * 下载
	 */
	protected List<Content> doQuery(
            HttpServletRequest request, HttpServletResponse response, Model model)
		throws Exception {

		Map<String, Object> params = getSearchParams(request);
		return getEntityService().query(params, getSortMap(request));
	}

	@RequestMapping(value = "saveJson")
	@ResponseBody
	public JsonEntityResult<Content> saveJson(
            HttpServletRequest request, HttpServletResponse response) {

		JsonEntityResult<Content> result = new JsonEntityResult<Content>();
		try {
	...
	@ResponseBody
	public JsonResult removeAttachment(
            HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			if (StringUtils.isNotBlank(id)) {
				...
				if (a != null) {
					filePath +=
						StringUtils.substringAfter(a.getFilePath(), "/media");
	...
	protected Content onSave(
            HttpServletRequest request, HttpServletResponse response, Model model,
            Content entity, boolean isCreate)
		throws Exception {
	...
	public String importJsonReview(
		...
		try {
			...
			if (uploadResults != null && uploadResults.size() > 0) {
				...
				Iterator<Entry<String, UploadResult>> iterator =
					uploadResults.entrySet().iterator();
