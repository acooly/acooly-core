...
public class MysqlTableLoader implements TableLoader {
     ...
     * 列相关元数据SQL
     */
    protected static final String COLUMN_METADATA_SQL =
            "select COLUMN_NAME as name,DATA_TYPE as type,(case when data_type='varchar' then CHARACTER_MAXIMUM_LENGTH else NUMERIC_PRECISION end)as length, IS_NULLABLE AS nullable,COLUMN_COMMENT AS comments,COLUMN_DEFAULT AS defaultValue,EXTRA AS extra from information_schema.COLUMNS where table_schema=? and table_name=? order by ORDINAL_POSITION";
    protected static final String ALL_COLUMN_METADATA_SQL =
     ...
     * 获取所有的表名SQL
     */
    protected static final String SELECT_ALL_TABLES =
            "select TABLE_NAME from information_schema.tables where table_schema=? ORDER BY TABLE_NAME";
    protected static final String SELECT_VERSION = "SELECT version()";
    ...
    /**
     * 检查id、createTime、updateTime 格式分别为： id bigint(20) NOT NULL AUTO_INCREMENT COMMENT 'ID',
     * create_time timestamp DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间', update_time timestamp DEFAULT
    ...
    public boolean checkTableColums(List<Column> columns) {
        ...
        for (Column column : columns) {
            ...
            if (StringUtils.equalsIgnoreCase("id", name)
                    && StringUtils.equalsIgnoreCase("bigint", type)
                    && StringUtils.equalsIgnoreCase("auto_increment", extra)) {
                isIdValid = true;
                continue;
            }
            //检查createTime
            if (StringUtils.equalsIgnoreCase("timestamp", type)
                    && StringUtils.equalsIgnoreCase("CURRENT_TIMESTAMP", defaultValue)) {
                if (StringUtils.equalsIgnoreCase("create_time", name)
                        || StringUtils.equalsIgnoreCase("raw_add_time", name)) {
                    hasCreateTime = true;
                    continue;
                }
            }
            ...
            if (StringUtils.equalsIgnoreCase("timestamp", type)
                    && StringUtils.equalsIgnoreCase("CURRENT_TIMESTAMP", defaultValue)
                    && StringUtils.equalsIgnoreCase("ON update CURRENT_TIMESTAMP", extra)) {
                if (StringUtils.equalsIgnoreCase("update_time", name)
                        || StringUtils.equalsIgnoreCase("raw_update_time", name)) {
                    hasUpdateTime = true;
                }
            }
        }
