 ...
 * 对源类型父类和目标类型子类查找到的转换器该管理器采用了缓存的机制并且没有串行化， 所以该
 * <strong>缓存在极端并发条件下可能出现不一致的情况</strong>。不过从使用场景上看，
...
public class SimpleTypeConverterManager implements TypeConverterManager {
	 ...
	 *            需要注册默认转换器的 TypeConverterManager 。
	 */
	...
	public <T> Collection<TypeConverter<T>> getTypeConverter(Class<T> targetType) {
		...
		return (Collection<TypeConverter<T>>) (m == null ? Collections.emptyList()
