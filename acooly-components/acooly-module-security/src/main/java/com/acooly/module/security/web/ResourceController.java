...
public class ResourceController extends AbstractJQueryEntityController<Resource, ResourceService> {
	...
	private static Set<String> loadIcon(HttpServletRequest request) {
		...
		InputStream in = null;
		try {
			AbstractFileResolvingResource resource = new ClassPathResource("static/manage/style/icon.css");
			if(!resource.exists()){
				resource = new ServletContextResource(request.getSession().getServletContext(),"/manage/style/icon.css");
			}
