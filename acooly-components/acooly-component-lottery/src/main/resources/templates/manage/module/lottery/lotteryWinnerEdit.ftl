...
<div>
    <form id="manage_lotteryWinner_editform"
          action="${rc.contextPath}/manage/module/lottery/lotteryWinner/<#if action == 'create'>save<#else>update</#if>Json.html"
          method="post">
              ...
              <tr>
                  ...
                  <td>
					...
					<#list allStatuss as key,val>
                        <option value="${key}" <#if allStatuss?? && allStatuss.status.code == key>selected</#if> >${val}</option></#list>
                      </select>
                  </td>
              </tr>
              <tr>
