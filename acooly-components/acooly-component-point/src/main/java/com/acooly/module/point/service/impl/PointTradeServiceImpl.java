...
public class PointTradeServiceImpl extends EntityServiceImpl<PointTrade, PointTradeDao>
    implements PointTradeService {
  ...
  @Autowired private PointStatisticsService pointStatisticsService;
      ...
      for (PointStatistics pointStatistics : pages.getPageResults()) {
        if (pointStatistics.getStatus() == PointStaticsStatus.init) {
          ...
          PointAccount pointAccount =
              pointAccountService.findByUserName(pointStatistics.getUserName());
          ...
          saveTrade(
              pointAccount, pointStatistics.getActualPoint(), PointTradeType.clear, pointTradeDto);
        }
      }
    }
  }

  private PointTrade saveTrade(
      PointAccount pointAccount,
      long point,
