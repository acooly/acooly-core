...
<div align="center">
    <form id="manage_portallet_editform" action="${rc.contextPath}/manage/system/portallet/<#if action == 'create'>save<#else>update</#if>Json.html" method="post">
      <@jodd.form bean="portallet" scope="request">
        <input name="id" type="hidden" />
        <table class="tableForm" width="100%">
			<tr>
				<th width="25%">题标</th>
				<td><input type="text" name="title" class="easyui-validatebox" data-options="required:true" validType="length[1,64]"/></td>
			</tr>
			<tr>
				<th>块高度：</th>
