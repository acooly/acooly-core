...
public abstract class AbstractFileOperationController<T extends AbstractEntity, M extends EntityService<T>> extends
	...
	protected List<T> doImportExcel(HttpServletRequest request, HttpServletResponse response, Model model)
			throws Exception {
		return doImport(request, response, model, FileType.EXCEL);
	}

	protected List<T> doImportCsv(Model model, HttpServletRequest request, HttpServletResponse response)
			throws Exception {
	 ...
	 * @throws Exception
	 */
	protected List<T> doImport(HttpServletRequest request, HttpServletResponse response, Model model, FileType fileType)
			throws Exception {
	...
	protected List<String[]> doImportLoad(Map<String, UploadResult> uploadResults, String encoding) {

		if (uploadResults == null || uploadResults.size() == 0) {
			return null;
		}
		FileType fileType = getFileType(uploadResults.entrySet().iterator().next().getValue());
		if (fileType == FileType.EXCEL) {
	 ...
	 * @return
	 */
	protected void doExportExcel(HttpServletRequest request, HttpServletResponse response, Model model)
			throws Exception {
	 ...
	 * @return
	 */
	protected void doExportCsv(HttpServletRequest request, HttpServletResponse response, Model model) throws Exception {
		// List<T> list = doQuery(request, response, model);
		// doExportCsvHeader(request, response);
		// doExportCsvBody(list, request, response);
		doExportCsv(request, response, model, getDefaultExportBatchSize());
	}

	protected void doExportCsv(HttpServletRequest request, HttpServletResponse response, Model model, int batchSize)
			throws Exception {
		doExportCsvHeader(request, response);
		doExportCsvBody(request, response, batchSize);
	}
	 ...
	 * @return
	 */
	protected boolean doExportExcelBody(List<T> list, HttpServletRequest request, HttpServletResponse response) {
	 ...
	 * @throws Exception
	 */
	protected void doExportCsvBody(List<T> list, HttpServletRequest request, HttpServletResponse response)
			throws Exception {
		PrintWriter p = null;
		try {
	...
	protected List<String> marshalEntity(T entity) {
	...
	protected String getExportFileName(HttpServletRequest request) {
		...
		if (StringUtils.isNotBlank(exportFileName)) {
			...
			// try {
			// fileName = new String(exportFileName.getBytes("UTF-8"),
	...
	public int getDefaultExportBatchSize() {
