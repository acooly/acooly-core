...
<div class="easyui-layout" data-options="fit : true,border : false">
    ...
    <form id="manage_user_searchform">
      <table class="tableForm" width="100%">
        <tr>
          ...
          状态: <select id="search_EQ_status" name="search_EQ_status" editable="false" panelHeight="auto" class="easyui-combobox" style="height:27px;" >
                 <option value="">所有</option>
                 <#list allStatus.keySet() as key><option value="${key}">${allStatus.get(key)}</option></#list>
          	  </select>
          类型:<select id="search_EQ_userType" name="search_EQ_userType" panelHeight="auto" editable="false" class="easyui-combobox" style="height:27px;">
              <option value="">所有</option>
  ...
  <div data-options="region:'center',border:false">
    <table id="manage_user_datagrid" class="easyui-datagrid" url="${pageContext.request.contextPath}/manage/system/user/listJson.html" toolbar="#manage_user_toolbar" fit="true" border="false" fitColumns="false"
