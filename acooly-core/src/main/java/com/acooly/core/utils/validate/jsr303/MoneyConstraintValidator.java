...
public class MoneyConstraintValidator extends ConstraintValidatorSupport
	...
	public boolean isValid(com.acooly.core.utils.Money value, ConstraintValidatorContext context) {
		...
		if (value == null) {
			if (!nullable) {
				if (hasCustomMessage()) {
					...
					context.buildConstraintViolationWithTemplate("{com.acooly.utils.validator.Money.message.notNull}")
		...
		if (value.getCent() < min) {
			if (hasCustomMessage()) {
				context.disableDefaultConstraintViolation();
				context.buildConstraintViolationWithTemplate("{com.acooly.utils.validator.Money.Min.message}")
				        .addConstraintViolation();
			}
		...
		if (value.getCent() >= max) {
			if (hasCustomMessage()) {
				context.disableDefaultConstraintViolation();
				context.buildConstraintViolationWithTemplate("{com.acooly.utils.validator.Money.Max.message}")
				        .addConstraintViolation();
			}
			return false;
		}
