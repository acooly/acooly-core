 ...
 * 
 * 迁移到：com.acooly.core.integration.log
 * 
 * @author zhangpu
 * 
 */
...
public class LogbackIntergrationListener implements ServletContextListener {
	private static final Logger logger = LoggerFactory
			.getLogger(LogbackIntergrationListener.class);
	...
	public void contextInitialized(ServletContextEvent event) {
		...
		if (StringUtils.isNotBlank(springActiveProfile)) {
			location = StringUtils.replace(locationConfig, "logback.xml",
					"logback." + springActiveProfile + ".xml");
		}
		...
		try {
			...
			if (locationFile.exists()) {
				servletContext
						.log("Initializing slf4j from [" + location + "]");
				...
				servletContext.log("logConfigLocation [" + location
						+ "] does not exist. use the default logger.");
			}
		} catch (Exception e) {
