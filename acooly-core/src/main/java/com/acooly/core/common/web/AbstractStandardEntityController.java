...
public abstract class AbstractStandardEntityController<
    ...
    private static final Logger logger =
            LoggerFactory.getLogger(AbstractStandardEntityController.class);
    /**
     * 导入视图
     */
    ...
    protected void onEdit(
            HttpServletRequest request, HttpServletResponse response, Model model, T entity) {
    }
    ...
    /**
     * 向View层传递message时将message放入httpSession的messages变量中. 放在session中能保证message即使Redirect也不会消失。 需配合
     * MessageFilter}使用, MessageFilter实现从Session中读取message并放入到request中 然后，从session中删除message
     */
    ...
    protected String getRequestMapperValue() {
        if (StringUtils.isBlank(requestMapperValue)) {
            String[] value =
                    (String[]) AnnotationUtils.getValue(getClass().getAnnotation(RequestMapping.class));
