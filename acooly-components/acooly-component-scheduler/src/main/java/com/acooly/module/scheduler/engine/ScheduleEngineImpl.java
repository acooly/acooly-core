...
public class ScheduleEngineImpl implements ScheduleEngine {
    ...
    public void deleteJob(SchedulerRule ruleDO) {
        try {
            ...
            throw Exceptions.runtimeException(getLogPrefix(ruleDO.getId()) + "删除失败:"
                + e.getMessage());
        }
    }
    ...
    private void init() {
        ...
        schedulerRules.forEach(schedulerRule -> {
    ...
    public void addJobOrUpdateTriggerToEngine(final SchedulerRule rule) throws SchedulerException {
        ...
        if (null == oldTrigger) {
            ...
            if (rule.getValidityStart().before(sysdate)) {
                newTrigger = newTrigger().withIdentity(triggerKey, groupKey)
                    .withSchedule(cronSchedule(rule.getCronString()).withMisfireHandlingInstructionDoNothing())
                    .endAt(rule.getValidityEnd()).build();
            } else {
                newTrigger = newTrigger().withIdentity(triggerKey, groupKey)
                    .withSchedule(cronSchedule(rule.getCronString()).withMisfireHandlingInstructionDoNothing())
                    .startAt(rule.getValidityStart()).endAt(rule.getValidityEnd()).build();
            }
            ...
            // 按新的trigger重新设置job执行
            if (rule.getValidityStart().before(sysdate)) {
                newTrigger = oldTrigger.getTriggerBuilder().withIdentity(triggerKey, groupKey)
                    .withSchedule(cronSchedule(rule.getCronString()).withMisfireHandlingInstructionDoNothing())
                    .endAt(rule.getValidityEnd()).build();
            } else {
                newTrigger = oldTrigger.getTriggerBuilder().withIdentity(triggerKey, groupKey)
