...
public class FinalizableReferenceQueue extends ReferenceQueue<Object> {
	...
	private static final Set<CleanUpThread> THREADS = Collections
			.newSetFromMap(new WeakHashMap<CleanUpThread, Boolean>());
	 ...
	 * 
	 * @param name
	 *            引用队列的名称，该名称用做清理的守护线程的名称。
	 */
	public FinalizableReferenceQueue(String name) {
		CleanUpThread thread = newCleanUpThread(name);
		this.threadRederence = new WeakReference<FinalizableReferenceQueue.CleanUpThread>(thread);
		start(thread);
	}
	...
	public static Set<CleanUpThread> getCleanUpThreads() {
		READ_LOCK.lock();
		try {
			return new LinkedHashSet<FinalizableReferenceQueue.CleanUpThread>(THREADS);
		} finally {
			READ_LOCK.unlock();
		}
	}
