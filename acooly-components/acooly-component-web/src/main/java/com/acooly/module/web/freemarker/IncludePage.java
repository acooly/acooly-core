...
/*
 * 修订记录:
 * qiubo@yiji.com 2017-03-29 15:35 创建
 */
...
public class IncludePage implements TemplateDirectiveModel {
	...
	public void execute(final Environment env, Map params, TemplateModel[] loopVars,
		...
		if (!(path instanceof TemplateScalarModel)) {
			throw new _MiscTemplateException(env, "Expected a scalar model. \"path\" is instead ",
		...
		if (envOut == response.getWriter()) {
			...
			final PrintWriter printWriter = (envOut instanceof PrintWriter) ? (PrintWriter) envOut
		...
		if (inheritParamsModel == null) {
			...
			if (!(inheritParamsModel instanceof TemplateBooleanModel)) {
				throw new _MiscTemplateException(env, "\"inherit_params\" should be a boolean but it's a(n) ",
