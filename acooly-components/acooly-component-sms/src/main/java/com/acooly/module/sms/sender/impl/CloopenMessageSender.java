 ...
 * @See com.acooly.core.test.web.TestController#testCloopenSms()
 */
...
public class CloopenMessageSender extends AbstractShortMessageSender {
    ...
    public String send(List<String> mobileNos, String content) {
        ...
        try {
            InputStream xmlSerialize = CloopenMessageSendSerializer.getInstance().serialize(params, "UTF-8");

            //2013-12-26/Accounts/{accountSid}/SMS/TemplateSMS?sig={SigParameter}
            HttpPost httppost = new HttpPost(SEND_URL_CLO + "2013-12-26/Accounts/" + accountId + "/SMS/TemplateSMS?sig=" + sign);
        ...
        if (statusCodeNode.getLength() > 0) {
            ...
            if (!StringUtils.isEmpty(statusCode)) {
                if (CloopenMessageResponseParser.SUCCESS_CODE.equals(statusCode)) {
                    ...
                    throw new BusinessException(CloopenMessageResponseParser.codeMapping.get(statusCode), statusCode);
                }
            }
        }
