...
public class RSACipherCache {
	...
	private static final ThreadLocal<LoadingCache<Key, Cipher>> threadLocal = new ThreadLocal<LoadingCache<Key, Cipher>>() {
		...
		protected LoadingCache<Key, Cipher> initialValue() {
			 ...
			 * 设置一个按容量控制的内存缓存
			 */
			return CacheBuilder.newBuilder().recordStats().maximumSize(THREAD_CACHE_SIZE)
					.build(new CacheLoader<Key, Cipher>() {
				@Override
