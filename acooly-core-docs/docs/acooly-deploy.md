...
|服务器 |ECS| 2核8G/CentOS7.x|2|java应用，单应用双节点负载均衡和故障转移，如果是测试环境，单机也可以
|数据库|RDS| 4核8G及以上/MySQL5.7+|1| MySQL双机集群，当然你可以自己搭建MySQL集群
...
如果你打算自建相关的基础服务（MySQL，Reids，NFS ,zookeeper等），请进来准备CentOS7.x的操作系统，参考2.1节点的工具安装说明，安装好工具包，待使用。

### 2.2.1 [可选] MySQL和Redis
...
在在Java应用服务器上，使用上节安装好的工具包里面的命令直接安装对应的软件和服务即可，下面介绍典型配置。
...
* 共享磁盘挂载，请输入共享NFS服务的局域网地址

>注意：脚本工具会自动创建和挂载到本地目录`/data`，当然你可以查看并调整挂载目录
...
在Java服务器和基础服务器配置完成后，登录Java服务器，按一下步骤发布应用。
...
* 拉取代码：每个项目部署时都需要先手动的完整拉取一次代码。请在/opt/app下运营git clone方式拉取。
	...
	# 首次clone需输入账号密码，然后配置存储账号，下次我们运行`start.sh`命令拉取（git pull）代码时，则不用在输入密码。
...
* 启动服务：在/opt/app下运行 `./start.sh ${projectName}` , 命令会自动更新代码，打包和启动
	...
	>注意：`start.sh ${projectName}`中的`projectName`一般就是你前面clone的工程的目录名称，注意不能带`\`
