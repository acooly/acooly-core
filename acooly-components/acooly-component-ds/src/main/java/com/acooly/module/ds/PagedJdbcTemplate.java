 ...
 * JdbcTemplate分页查询扩展
 *
 * <p>废弃原来的低版本的ObjectRowMapper扩展 查询结果通过RowMapper直接映射为对象(映射基本规则使用查询的列名与对象的属性名匹配，并实现自动类型转换)
 *
 * <p>为了简单，如果是复杂多表关联查询，请使用视图。
 *
 * @author zhangpu
...
public class PagedJdbcTemplate extends JdbcTemplate {
    ...
    private static final String ORACLE_PAGESQL_TEMPLATE =
            "SELECT * FROM (SELECT XX.*,rownum ROW_NUM FROM (${sql}) XX ) ZZ where ZZ.ROW_NUM BETWEEN ${startNum} AND ${endNum}";
     ...
     */
    @Deprecated
    public <T> PageInfo<T> queryMSSQL(
            PageInfo<T> pageInfo, String sql, Class<T> dtoEntity, String pageColumn) {
        try {
