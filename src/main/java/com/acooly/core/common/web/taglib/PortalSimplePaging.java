...
public class PortalSimplePaging extends TagSupport {
	 ...
	 * 表单提交
	 */
	private static final String PAGE_SCRIPT = "<script language=\"JavaScript\">" + "function goToByLabel(f, label){\n"
			+ " var currentPage = parseInt(document.getElementById('currentPage').value);" + " if(label==\"last\"){"
			+ "  currentPage=parseInt(document.getElementById('totalPage').value);" + " }"
			+ " else if(label==\"first\"){" + "  currentPage=1;" + " }" + " else if(label==\"previous\"){"
			+ "  currentPage--;" + " }" + " else if(label==\"next\"){" + "  currentPage++;" + " }" + " else {"
			+ "  currentPage=parseInt(label);" + " }" + " document.getElementById('currentPage').value = currentPage;"
			+ "	if(&checkMethod&){" + "	f.submit();\n" + "	}" + "}" + "" + "" + "" + "" + "" + "</script>";
	...
	private PageInfo<Object> getPageInfo() throws JspException {
		...
		if (obj != null) {
			if (PropertyUtils.isReadable(obj, "countOfCurrentPage") && PropertyUtils.isReadable(obj, "currentPage")
					&& PropertyUtils.isReadable(obj, "totalPage") && PropertyUtils.isReadable(obj, "totalCount")
	...
	private Map<Object, Object> convertProperties(String prop, String separator, String subSeparator) {
	...
	private String makeButton(String label, String value, boolean disable) {
		String style = "type:button;" + "value=\"\"" + " class=\"" + label + "\"" + " onclick:goToByLabel(document."
				+ this.name + ",'" + value + "');" + (disable ? "disabled:disabled" : "");
		String button = this.makeStartTag("input", this.buttonParam + ";" + style);
		return button;
	}

	private String makeHref(String pageNo) {
		String href = "<a href=\"javascript:goToByLabel(document.getElementById('" + this.name + "'),'" + pageNo
				+ "')\"";
	...
	public int doStartTag() throws JspException {
		...
		try {
			...
			out.println(this.makeHidden("currentPage", "currentPage", String.valueOf(pageInfo.getCurrentPage())));
			out.println(this.makeHidden("totalPage", "totalPage", String.valueOf(pageInfo.getTotalPage())));
