 ...
 * 观察者处理Subject
 * 
 * 具体项目中，如果需要对Shire安全框架中的登录，注销动作进行后续处理，可以实现LoginLogoutListener，
 * 并配置到Spring托管。该类会自动通知登录后和注销前两个状态。
 * 
 * @author zhangpu
 * 
 */
public class ShireLoginLogoutSubject implements LoginLogoutListener,
		ApplicationContextAware, InitializingBean {

	private static final Logger logger = LoggerFactory
			.getLogger(ShireLoginLogoutSubject.class);
	...
	public void beforeLogout(HttpServletRequest request,
		...
		for (Map.Entry<String, LoginLogoutListener> entry : listeners
				.entrySet()) {
	...
	private void notifyBeforeLogout(HttpServletRequest request,
			HttpServletResponse response, Subject subject,
	...
	private void notifyAfterLogin(AuthenticationToken token,
			AuthenticationException e, HttpServletRequest request,
			HttpServletResponse response, LoginLogoutListener listener) {
		try {
	...
	public void afterPropertiesSet() throws Exception {
		listeners = applicationContext
				.getBeansOfType(LoginLogoutListener.class);
		Iterator<Map.Entry<String, LoginLogoutListener>> iterator = listeners
				.entrySet().iterator();
