 ...
 * <p>
 * 该 {@link Set} 的特性与被包装的 {@link Map#keySet()} 相同但是支持 add 操作（除非被包装的
 * {@link Map#put(Object, Object)} 本身不可用），底层实现是调用
 * {@link Map#put(Object, Object)} 。对于 value ，会使用 {@link #defaultV}。
 * 
...
public class SetFromMap<E, V> extends AbstractSet<E> implements Set<E>, Serializable {
	...
	/**
	 * 使用 {@link #SetFromMap(Map)} 构造 {@link SetFromMap} 时，调用
	 ...
	 *            被包装的 {@link Map}。
	 * @param defaultV
	 *            在调用 {@link #add(Object)} 时所使用作为被包装 {@link Map} 的 value 。
	 * @throws NullPointerException
	 *             如果 map 为 null。
	 */
