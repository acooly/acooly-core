...
public class ToString {
	...
	public static class Generator {
		...
		private void generateBegin() {
			...
			// 强制转换源对象
			String convertSource = String.format("%s %s=(%s)xxoo;", source.getName(), SOURCE, source.getName(), SOURCE);
		...
		private String buildReadMethod(PropertyDescriptor pd) {
			...
			if (toStringMethod != null) {
				readMethod = String.format("%s==null?null:%s.%s()", readMethod, readMethod, toStringMethod.value());
			}
			return readMethod;
		}
		...
		private void buildCollection(String toStringClassName, String readMethod, String propName) {
			readMethod = String.format("%s.toString((java.util.Collection)%s,%d)", toStringClassName, readMethod,
					TO_STRING_COLLECTION_SIZE_THRESHOLD);
			doProp(propName, readMethod);
		}
		...
		private void buildMap(String toStringClassName, PropertyDescriptor pd, String readMethod, String propName) {
			...
			if (maskable == null) {
				readMethod = String.format("%s.toString((java.util.Map)%s,%d,null)", toStringClassName, readMethod,
						TO_STRING_MAP_SIZE_THRESHOLD);
			} else {
				List<String> maskKeyList = (maskable.maskKeys() == null || maskable.maskKeys().length == 0) ? null
						: Arrays.asList(maskable.maskKeys());
				readMethod = String.format("%s.toString((java.util.Map)%s,%d,%s)", toStringClassName, readMethod,
						TO_STRING_MAP_SIZE_THRESHOLD, maskKeyField(maskKeyList));
			}
			doProp(propName, readMethod);
		}
		...
		private void buildString(String toStringClassName, PropertyDescriptor pd, String readMethod, String propName) {
			Maskable maskable = getAnnotation(pd, Maskable.class);
			if (maskable == null) {
				readMethod = String.format("%s.toString((String)%s,%d)", toStringClassName, readMethod,
						TO_STRING_ARRAY_SIZE_THRESHOLD);
