...
public class LotteryFacadeImpl implements LotteryFacade {
    ...
    public ResultBase addLotteryCount(LotteryCountOrder order) {
        ...
        try {
            ...
            lotteryUserCountService.appendTimes(lottery.getId(), lottery.getCode(), lottery.getTitle(), order.getUsername(), order.getCount());
    ...
    public LotteryCountResult getLotteryCount(LotteryCountOrder order) {
        ...
        try {
            order.check();
            LotteryUserCount lotteryUserCount = lotteryUserCountService.findUniqueUser(order.getLotteryCode(), order.getUsername());
            BeanCopier.copy(lotteryUserCount, result, BeanCopier.CopyStrategy.IGNORE_NULL, BeanCopier.NoMatchingRule.IGNORE);
