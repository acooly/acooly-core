...
public abstract class AbstractOperationController<T extends Entityable, M extends EntityService<T>>
	...
	protected PageInfo<T> doList(HttpServletRequest request, HttpServletResponse response, Model model)
			throws Exception {
		return getEntityService().query(getPageInfo(request), getSearchParams(request), getSortMap(request));
	}
	 ...
	 * @throws Exception
	 */
	protected T onSave(HttpServletRequest request, HttpServletResponse response, Model model, T entity,
