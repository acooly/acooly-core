 ...
 * 
 * 迁移到:com.acooly.core.integration
 * 
 * @author zhangpu
 * 
 */
@Deprecated
public class SystemPropertyIntergrationListener implements
	...
	private void setSpringProfileActiveSystemProperty(
			ServletContext servletContext) {
		String profile = System.getProperty(SPRING_PROFILE_ACTIVE_KEY_PARAM);
		if (StringUtils.isNotBlank(profile)) {
			servletContext.log("Use System property "
					+ SPRING_PROFILE_ACTIVE_KEY_PARAM + " = " + profile);
			return;
		}

		profile = servletContext
				.getInitParameter(SPRING_PROFILE_ACTIVE_KEY_PARAM);
		if (StringUtils.isBlank(profile)) {
			servletContext
					.log("env setting["
							+ SPRING_PROFILE_ACTIVE_KEY_PARAM
							+ "] does not exist form system-property and web-context-param");
			return;
		}
		System.setProperty(SPRING_PROFILE_ACTIVE_KEY_PARAM, profile);
		servletContext.log("Use context-param "
				+ SPRING_PROFILE_ACTIVE_KEY_PARAM + " = " + profile);
	}
