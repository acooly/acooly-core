...
@Table(
  name = "lottery",
  schema = "",
  indexes = {@Index(name = "uk_lottery_code", unique = true, columnList = "code")}
)
public class Lottery extends AbstractEntity {
  ...
  @Column(
    ...
    length = 512,
    nullable = true,
    columnDefinition = "varchar(512) comment '说明'"
  )
  private String note;
  ...
  @Column(
    ...
    length = 16,
    nullable = false,
    columnDefinition = "varchar(16) not null comment'类型'"
  )
  ...
  @Column(
    name = "start_time",
    columnDefinition = "datetime NOT NULL DEFAULT CURRENT_TIMESTAMP  COMMENT '开始时间'"
  )
  ...
  @Column(
    name = "end_time",
    columnDefinition = "datetime NOT NULL DEFAULT CURRENT_TIMESTAMP  COMMENT '结束时间'"
  )
  ...
  @Column(
    ...
    length = 16,
    nullable = false,
    columnDefinition = "varchar(16) not null comment'是否参与计数'"
  )
  @Enumerated(EnumType.STRING)
